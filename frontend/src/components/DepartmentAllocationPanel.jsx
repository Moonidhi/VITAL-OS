 ...
 *
 * Displays AI-driven power allocation across all 7 hospital departments.
 * Receives `snapshot` (from /simulation/current) and `prediction`
 * (from /ai/predict) as props — does NO internal polling.
 *
 * Allocation logic is a pure function of (priorityTier, riskLevel):
 *   LOW  → all departments 100%
 *   MEDIUM → Low-priority depts begin reducing
 *   HIGH  → only Critical/High remain fully powered
 */

// ── Department registry ──────────────────────────────────────────────────────
// snapshotKey matches exactly what /simulation/current returns
// (prefixed with "load_" per SimulationSnapshot.to_dict())
...
const DEPARTMENTS = [
  ...
  {
    key:      'load_Operation_Theatre',
    label:    'Operation Theatre',
    tier:     'Critical',
    color:    '#FF5C5C',
  },
  {
    key:      'load_Emergency_Department',
    label:    'Emergency Dept',
    tier:     'Critical',
    color:    '#F5A623',
  },
  {
    key:      'load_Oxygen_Plant',
    label:    'Oxygen Plant',
    tier:     'High',
    color:    '#4DD0C4',
  },
  {
    key:      'load_General_Ward',
    label:    'General Ward',
    tier:     'Medium',
    color:    '#7C9EFF',
  },
...
// ── Allocation table ─────────────────────────────────────────────────────────
// (priorityTier, riskLevel) → { powerPercent, status }
...
function DepartmentCard({ dept, currentKw, riskLevel }) {
  ...
  // Power bar: full width = current load, filled portion = allocated portion
  const barFill = powerPercent

  return (
    <div
      className="bg-base-elevated rounded-xl border p-3.5 flex flex-col gap-2.5 transition-all duration-500"
      style={{ borderColor: powerPercent === 0 ? '#FF5C5C30' : '#252E3D' }}
    >
      {/* ── Header row ── */}
      ...
      <div className="grid grid-cols-3 gap-1 pt-0.5">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-[11px] font-mono text-text-primary">
            {allocatedKw.toFixed(1)}{' '}
            <span className="text-text-faint text-[9px]">kW</span>
          </span>
        </div>
      </div>
    </div>
...
export default function DepartmentAllocationPanel({ snapshot, prediction }) {
  ...
  // Compute totals for the summary footer
  ...
  return (
    <div className="bg-base-surface rounded-xl border border-base-border shadow-card p-4">
      ...
      {/* ── Strategy banner ── */}
      <div
        className="rounded-lg px-3 py-2 mb-4 text-[11px] font-medium"
      ...
      <div className="mt-4 pt-3 border-t border-base-border grid grid-cols-3 gap-4">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="font-mono text-sm text-text-primary">
            {totalCurrentKw.toFixed(1)}{' '}
            <span className="text-text-faint text-xs">kW</span>
          </span>
        </div>
        <div className="flex flex-col gap-0.5">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <span
            ...
            style={{ color: shedKw > 0 ? '#FF5C5C' : '#3DD68C' }}
          >
            {shedKw.toFixed(1)}{' '}
            <span className="text-text-faint text-xs">kW</span>
          </span>
